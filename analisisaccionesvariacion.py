...
def create_monthly_heatmap(monthly_data, main_ticker, second_ticker, third_ticker):
  ...
  # Define custom colormap (red for negative, green for positive)
...
def analyze_streaks(monthly_data, main_ticker):
  ...
  positive_streaks = [s for s in streaks if s['value'] > 0]
  negative_streaks = [s for s in streaks if s['value'] <= 0]
  ...
  with col1:
      ...
      if positive_streaks:
          max_positive = max(positive_streaks, key=lambda x: x['length'])
  ...
  with col2:
      ...
      if negative_streaks:
          max_negative = max(negative_streaks, key=lambda x: x['length'])
