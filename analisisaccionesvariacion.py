...
# Function to add a watermark to matplotlib figures
def add_watermark(fig):
    fig.text(0.5, 0.02, 'MTaurus - X: MTaurus_ok', ha='center', va='center', 
             fontsize=12, color='gray', alpha=0.5, style='italic')

# Function to add a watermark to Plotly figures
...
if data:
    ...
    if ratio_data is not None:
        ...
        add_plotly_watermark(fig)  # Add watermark to Plotly figure
        st.plotly_chart(fig)
        ...
        ax.legend()
        add_watermark(fig)  # Add watermark to matplotlib figure
        st.pyplot(fig)
        ...
        sns.heatmap(monthly_pivot, cmap=custom_cmap, center=0, annot=True, fmt=".1f", linewidths=.5, ax=ax)
